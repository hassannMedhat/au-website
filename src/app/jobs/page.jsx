...
export default function JobsPage() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    const { realName, cityName, discordId, nationalId, reason, dailyHours } =
      formData;
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        }, 2500); // علامة الصح تظهر لمدة 2.5 ثانية
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-black to-indigo-900 text-white w-full p-0 m-0">
      <div className="absolute inset-0 pointer-events-none">
        <div className="w-1 h-1 bg-white rounded-full absolute top-10 left-20 animate-twinkle"></div>
        <div className="w-2 h-2 bg-white rounded-full absolute top-40 right-30 animate-twinkle delay-200"></div>
        <div className="w-1 h-1 bg-white rounded-full absolute bottom-20 left-40 animate-twinkle delay-400"></div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 max-w-7xl mx-auto z-10">
        {jobsList.map((job) => (
          <div
            key={job.id}
            className="bg-gray-800 bg-opacity-80 backdrop-blur-md rounded-xl shadow-xl p-6 flex flex-col items-center transition-all duration-300 hover:scale-110 hover:rotate-2 hover:shadow-2xl hover:bg-opacity-100"
          >
            ...
            {job.soon ? (
              <div className="bg-red-600 text-white px-4 py-2 rounded-full animate-bounce">
                Coming Soon
              </div>
            ) : (
              <button
                onClick={() => handleApplyClick(job)}
                className="bg-gradient-to-r from-blue-500 to-purple-600 text-white px-6 py-2 rounded-full hover:from-blue-600 hover:to-purple-700 transition-all duration-300 hover:shadow-lg"
              >
                Apply Now
              </button>
            )}
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75 backdrop-blur-sm z-50 p-4 overflow-y-auto">
          <div className="bg-gray-900 bg-opacity-90 rounded-xl p-8 w-full max-w-lg max-h-[90vh] overflow-y-auto transform transition-all scale-100 animate-fadeIn">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                className="w-full bg-gradient-to-r from-blue-500 to-purple-600 text-white px-6 py-3 rounded-full hover:from-blue-600 hover:to-purple-700 transition-all duration-300 hover:shadow-lg"
                disabled={isLoading}
              >
      ...
      {showSuccessAnimation && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75 z-50">
          <div className="bg-green-600 bg-opacity-90 rounded-lg p-8 text-white text-center">
            <svg
